...
import type { PublicBankSettings } from "@/lib/order-queries";
...
export function CheckoutSuccessClient({
  ...
  bankSettings: PublicBankSettings;
  initialEmail?: string;
}) {
  ...
  useEffect(() => {
    const stored = sessionStorage.getItem(`checkout_email_${orderCode}`);
    if (stored) {
      setEmailForVerification(stored);
  ...
  const shippingCostValue = order.shippingCost;
  const discountValue = order.discountAmount;
  const couponCodeParam = order.couponCode;
