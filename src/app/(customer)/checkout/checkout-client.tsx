...
import type { AddressData } from "@/components/checkout/address-fields";
...
function AddressFieldsSkeleton() {
  return (
    <div className="space-y-5">
      <Skeleton className="h-14 w-full rounded-full bg-[#dcd0bf]" />
      <Skeleton className="h-14 w-full rounded-full bg-[#dcd0bf]" />
      ...
      <Skeleton className="h-14 w-full rounded-full bg-[#dcd0bf]" />
      <Skeleton className="h-14 w-full rounded-full bg-[#dcd0bf] sm:w-1/2" />
    </div>
  );
}
...
const AddressFields = dynamic(
  () =>
    import("@/components/checkout/address-fields").then((mod) => ({
      default: mod.AddressFields,
    })),
  {
    loading: () => <AddressFieldsSkeleton />,
    ssr: false,
  },
);
