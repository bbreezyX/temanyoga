        ...
        <div className="lg:col-span-3">
          <div className="rounded-[32px] bg-white shadow-soft ring-1 ring-warm-sand/30 overflow-hidden flex flex-col h-[800px]">
            ...
            <iframe
              ...
              title="Email Preview"
              onLoad={(e) => {
                // The iframe is a separate document, so globals.css can't reach
                // it — inject the brand scrollbar here (preview-only, same-origin).
                const doc = e.currentTarget.contentDocument;
                if (!doc) return;
                const style = doc.createElement("style");
                style.textContent =
                  "html{scrollbar-width:thin;scrollbar-color:#e8dcc8 transparent}" +
                  "::-webkit-scrollbar{width:10px;height:10px}" +
                  "::-webkit-scrollbar-track{background:transparent}" +
                  "::-webkit-scrollbar-thumb{background-color:#e8dcc8;border-radius:9999px;border:2px solid transparent;background-clip:padding-box}" +
                  "::-webkit-scrollbar-thumb:hover{background-color:#c85a2d}";
